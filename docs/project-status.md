...
| Compiler-plugin PC-context evidence | `EVIDENCE_PARTIAL` | Alpha-3 SNAPSHOT implements a CLI-only same-request selected-Compile receipt and exact stable Scala 3 bounded child inspector. On the frozen Macro-Paradise Scala 3.8.4/JDK-25 case, C/M/F/S all resolved with exact containing ranges, source/artifact hashes, and no plugin replay; the live classpath-only PC control still resolved only C and now carries explicit non-causal context warnings. One maintained case does not establish READY, live plugin replay, whole-workspace atomicity, Scala 2/nightly/custom compiler support, a security sandbox, or general compiler-plugin semantics. |
...
Skill adoption remains partial rather than READY even though the scoped skill
discovery/install paths are ready. Scala target compatibility
now includes complementary evidence from two real projects but remains partial;
the observed multi-project routing gap is closed in alpha-3 SNAPSHOT
development. The next admitted semantic implementation is bounded post-compile
TASTy evidence plus Presentation Compiler context provenance; broader
compiler-plugin semantics remain outside the current evidence.
