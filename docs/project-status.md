...
The next evidence milestone is external feedback from real Scala projects and
concrete decisions, reported with the bounded comparison and reproducibility
context in [`early-feedback.md`](early-feedback.md). The most useful cases show
either a missing decision fact or materially better composition of facts that
strong compiler, build/test, IDE/LSP, or standard artifact tooling can already
provide. Installation/client friction and exact Scala/sbt/JDK/project
compatibility failures are also in scope.
