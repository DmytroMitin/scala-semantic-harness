...
| Scala target compatibility | `EVIDENCE_PARTIAL` | The fixture matrix is now supplemented by one maintained real Scala 2.13.18 project at a frozen upstream commit, yielding the bounded qualifier `EVIDENCE_PARTIAL_REAL_PROJECT_EVIDENCE`. Its unchanged root build passed compile/errors and all 116 tests through exact public alpha-2; syntax-first parsing, bounded dynamic symbol/type evidence, truthful degraded point evidence without static artifacts, and the exact-eight MCP projection also passed under JDK 21. The checked-in target produced no SemanticDB. One project does not establish broad Scala 2 dialect, macro, compiler-plugin, classpath, or ecosystem compatibility. |
...
now includes bounded real-project evidence but remains partial; the next useful
compatibility step is a second independent maintained Scala 2.13 project, not
a feature manufactured from the first target's absent SemanticDB configuration.
