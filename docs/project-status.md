...
The Alpha 3 release source-paired SemanticDB v2 correction has one
...
The Alpha 3 release target-aware source mapping v4 preserves the same
...
immutable Maven Central primary files byte-for-byte. The Alpha 3 lightweight
tag and GitHub prerelease identify commit
`075a60bfb7d7677d7fdfcc2369c9ffe41c8b32a8`, tree
`6b9a0a623e91e5a71979fa159ec57bd5609d3fef`; two clean builds reproduced all
32 public Central primaries. Its GitHub prerelease has generated source
archives and zero uploaded project assets. Mutable source `main` reports
`0.1.0-alpha.4-SNAPSHOT` for development only. There is no Alpha 4 Central
artifact, supported channel, tag, GitHub Release, or release-readiness claim.
The checked-in channel remains pinned to the exact eight Alpha 3 modules.
Fresh outsider-like installation from the
...
Strict-v6 freshness is mechanically qualified on its bounded cats-tagless case,
and one later agent naturally selected it for a stale-output decision. The
attempted causal A/C comparison did not yield a valid comparison: its
preregistered isolation method failed its conjunctive qualification after
shared per-user JVM state was consulted outside the declared cell tree. The
benchmark family was stopped before scored paths and is closed rather than
iteratively weakening the contract. No causal superiority claim follows, and
the method result is not a semantic-scala product defect.
...
Recent bounded decision-gap intakes now cover one current compiler-plugin
project and three foreign Scala OSS projects. The first screened five material
decisions and admitted no semantic gap. The second screened ten real decisions
across Chimney, Scala CLI, and Cats Effect, deep-screened two, and again admitted
no gap after strong project-native baselines remained the more direct authority.
These are bounded negative results, not a claim that semantic-scala is generally
redundant or that complex compiler-plugin work cannot yield a future gap.

External early-user feedback is therefore the primary next admission source,
reported with the concrete-decision, strong-baseline, and reproducibility
context in [`early-feedback.md`](early-feedback.md). Repeated repository-batch
mining is not the immediate gap-generation strategy. Reports of a real failure,
unsafe choice, ambiguity, material orchestration burden, no added value, or
compatibility/adoption friction are all useful. This emphasis is not a freeze on
maintenance, concrete compatibility fixes, documentation fixes, or other
...
the public `main` channel and preserved at its immutable tag and GitHub
prerelease. Source development is Alpha 4 SNAPSHOT. External early-user
feedback remains the primary next semantic-value input.
