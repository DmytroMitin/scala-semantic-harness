...
| Which explicit artifacts contain one source? | `semanticdb-for-source --file <scala> --workspace <dir> [--sbt-project <id>] [--sbt-scala-version <version>] [--sbt-java-home <absolute-directory>] --json` | none | CLI-only. No target is v2; opt-in target v4 adds authoritative target ownership through a root-only receipt. The optional axis requires the project and must match the effective receipt axis. Fresh is not build or coverage proof. |
...
| Target-aware `point-evidence` | Acquires the existing v3 classpath-bearing Compile receipt. Evaluating checked-in sbt build/plugin code may resolve dependencies, populate caches, or compile transitively through `Compile / fullClasspath`. It has no Scala-axis selector and remains unqualified pending a separate v4 slice. | Approval must cover those effects. It does not authorize arbitrary sbt commands. |
